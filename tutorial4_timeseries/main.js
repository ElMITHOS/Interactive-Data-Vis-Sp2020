...
const width = window.innerWidth * 0.7,
  ...
  radius = 7,
  default_selection = "Select a Estado";
...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.Total)])
  ...
  const selectElement = d3.select("#dropdown").on("change", function() {
    console.log("new selected Estado is", this.value);
    // `this` === the selectElement
    // this.value holds the dropdown value a user just selected
    state.selectedEstado = this.value;
  ...
  selectElement
    .selectAll("option")
    .data([
      ...Array.from(new Set(state.data.map(d => d.Estado))),
      default_selection,
    ])
    .join("option")
    .attr("value", d => d)
...
function draw() {
  ...
  let filteredData;
  if (state.selectedEstado !== null) {
    filteredData = state.data.filter(d => d.Estado === state.selectedEstado);
  }
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.Total)) // started from the bottom, now we're here ...Lol!
    );
